...
const DEMO_BADGES = [
  { id: "b1", emoji: "🌱", tier: "1",       label: "นักเขียนหน้าใหม่", tooltip: "สร้างหนังสือเล่มแรก" },
  { id: "b2", emoji: "🏆", tier: "3",       label: "เชฟมือโปร",        tooltip: "เพิ่มสูตรอาหารครบ 50 สูตร" },
  { id: "b3", emoji: "⭐", tier: "special", label: "ผู้สนับสนุน",      tooltip: "สมาชิกระดับพิเศษ" },
  { id: "b4", emoji: "📚", tier: "4",       label: "บรรณารักษ์",       tooltip: "มีหนังสือสาธารณะครบ 5 เล่ม" },
];

type DemoBadge = typeof DEMO_BADGES[number];

function ShowcaseBadge({ badge, pinnedId, onPin, onUnpin }: {
  badge: DemoBadge;
  ...
  useEffect(() => {
    ...
    function onMouseDown(e: MouseEvent) {
      if (btnRef.current && !btnRef.current.contains(e.target as Node)) onUnpin(badge.id);
    }
    document.addEventListener("mousedown", onMouseDown);
    return () => document.removeEventListener("mousedown", onMouseDown);
  }, [pinned, badge.id, onUnpin]);
  ...
  return (
    <HoverCard
      ...
      onOpenChange={(v) => {
        setHovered(v);
        if (v && pinnedId !== null && pinnedId !== badge.id) onUnpin(pinnedId);
      }}
      openDelay={200}
      closeDelay={100}
    >
      <HoverCardTrigger asChild>
        <button
          ref={btnRef}
          type="button"
          onClick={() => pinned ? onUnpin(badge.id) : onPin(badge.id)}
          className={`w-10 h-10 rounded-full flex items-center justify-center text-xl select-none shadow-sm transition-transform hover:scale-110 active:scale-95 ${TIER_BG[badge.tier]}`}
        >
          {badge.emoji}
        </button>
      </HoverCardTrigger>
      <HoverCardContent className="p-3 w-44 text-left">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-semibold text-foreground leading-tight">{badge.label}</span>
        </div>
        <span className={`inline-block text-[10px] font-medium px-2 py-0.5 rounded-full mb-2 ${TIER_PILL[badge.tier]}`}>
          {badge.tier === "special" ? "Special" : `Tier ${badge.tier}`}
        </span>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3">
        ...
        <span className="text-[10px] font-bold text-muted/70 uppercase tracking-[0.2em] px-1">{label}</span>
        <div className="h-px flex-1 bg-border" />
      </div>
      {children}
    </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="space-y-4">
      <h2 className="text-xs font-bold text-muted uppercase tracking-widest border-b border-border pb-2">{title}</h2>
      {children}
    </section>
  );
}
...
export default function UIShowcaseClient() {
  ...
  const [checkList, setCheckList]       = useState([
    ...
    { id: "3", label: "น้ำตาล 3 ช้อนโต๊ะ",  done: false },
  ...
  return (
    <div className="space-y-12">
      ...
      {/* ════════════════════════════════════════════════════════ */}
      <SectionGroup label="Typography & Visual">
      ...
      {/* ════════════════════════════════════════════════════════ */}
      <SectionGroup label="Interactive Controls">

        <Section title="Buttons">
          <div className="bg-surface rounded-2xl border border-border p-5 flex flex-wrap gap-3 items-center">
            <button className="px-4 py-2 rounded-xl bg-orange-500 text-white text-sm font-medium hover:bg-orange-600 transition-colors">Primary</button>
            <button className="px-4 py-2 rounded-xl bg-elevated border border-border text-sm text-secondary hover:bg-border transition-colors">Secondary</button>
            <button className="px-4 py-2 rounded-xl text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors border border-red-200 dark:border-red-900/30">Danger</button>
            <button className="px-4 py-2 rounded-xl text-sm text-orange-500 hover:text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-950/20 transition-colors">Ghost</button>
            ...
            <button disabled className="px-4 py-2 rounded-xl bg-orange-500 text-white text-sm font-medium opacity-40 cursor-not-allowed">Disabled</button>
            <button className="p-2 rounded-xl bg-elevated border border-border text-muted hover:text-foreground transition-colors"><Info className="w-4 h-4" /></button>
            <button className="p-2 rounded-xl text-muted hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors"><X className="w-4 h-4" /></button>
          </div>
        </Section>
          ...
          <div className="bg-surface rounded-2xl border border-border p-5 space-y-3">
            {(["a", "b", "c"] as const).map((k, i) => (
              <label key={k} className="flex items-center gap-3 cursor-pointer group">
                <button type="button" onClick={() => setChecks(p => ({ ...p, [k]: !p[k] }))}
        ...
        <Section title="Radio Button">
          <div className="bg-surface rounded-2xl border border-border p-5 space-y-3">
            {[{ id: "r1", label: "ตัวเลือก 1" }, { id: "r2", label: "ตัวเลือก 2" }, { id: "r3", label: "ตัวเลือก 3" }].map(opt => (
              <label key={opt.id} className="flex items-center gap-3 cursor-pointer group">
                <button type="button" onClick={() => setRadio(opt.id)}
                ...
                <span className={`text-sm transition-colors ${radio === opt.id ? "text-foreground font-medium" : "text-secondary"}`}>{opt.label}</span>
              </label>
            ))}
            <label className="flex items-center gap-3 opacity-40 cursor-not-allowed">
        ...
        <Section title="Check List">
          <div className="bg-surface rounded-2xl border border-border overflow-hidden">
            {/* layout on wrapper enables smooth reordering animation */}
            <motion.div layout className="divide-y divide-border">
              {sortedList.map(item => (
                <motion.div key={item.id} layout transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}>
                  <button
                    type="button"
                    onClick={() => setCheckList(p => p.map(x => x.id === item.id ? { ...x, done: !x.done } : x))}
                    className="w-full flex items-center gap-3 px-5 py-3 text-sm text-left hover:bg-elevated/60 transition-colors"
                  >
            ...
            <div className="px-5 py-2.5 border-t border-border bg-elevated/30">
              <p className="text-xs text-muted">
                เสร็จ {checkList.filter(x => x.done).length}/{checkList.length} รายการ · รายการที่ติ๊กแล้วจะเลื่อนลงล่างอัตโนมัติ
              </p>
            </div>
          </div>
        </Section>
      ...
      {/* ════════════════════════════════════════════════════════ */}
      <SectionGroup label="Form Inputs">
        ...
        <Section title="Text Inputs">
          <div className="bg-surface rounded-2xl border border-border p-5 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input placeholder="ใช้ใน form แก้ไข…"
                className="w-full border border-orange-300 rounded-xl px-3 py-2 text-sm bg-surface text-foreground focus:outline-none focus:ring-1 focus:ring-orange-400" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted">Textarea</label>
              <textarea rows={3} placeholder="คำอธิบาย…"
          ...
          <div className="bg-surface rounded-2xl border border-border p-5 space-y-4">
            ...
            <div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {tabComp === "tab-a" && <p className="text-sm text-secondary">เนื้อหาของ <span className="font-semibold text-foreground">Tab A</span> — ภาพรวมสูตรอาหาร</p>}
                  {tabComp === "tab-b" && <p className="text-sm text-secondary">เนื้อหาของ <span className="font-semibold text-foreground">Tab B</span> — รายการวัตถุดิบ</p>}
                  {tabComp === "tab-c" && <p className="text-sm text-secondary">เนื้อหาของ <span className="font-semibold text-foreground">Tab C</span> — ขั้นตอนการทำ</p>}
                </motion.div>
              </AnimatePresence>
            </div>
          </div>
      ...
      {/* ════════════════════════════════════════════════════════ */}
      <SectionGroup label="Overlays & Popovers">
              ...
              <div className="flex flex-wrap gap-3 items-center">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button className="px-4 py-2 rounded-xl bg-elevated border border-border text-sm text-secondary hover:bg-border transition-colors">Tooltip บน (top)</button>
                  </TooltipTrigger>
                  <TooltipContent side="top">แสดงจากด้านบน</TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button className="px-4 py-2 rounded-xl bg-elevated border border-border text-sm text-secondary hover:bg-border transition-colors">Tooltip ล่าง (bottom)</button>
                  </TooltipTrigger>
                  <TooltipContent side="bottom">แสดงจากด้านล่าง</TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button className="px-4 py-2 rounded-xl bg-elevated border border-border text-sm text-secondary hover:bg-border transition-colors">Tooltip ขวา (right)</button>
                  </TooltipTrigger>
                ...
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    <button className="px-4 py-2 rounded-xl bg-orange-500 text-white text-sm font-medium hover:bg-orange-600 transition-colors">No Delay</button>
                  </TooltipTrigger>
                  <TooltipContent side="top">delayDuration=0 — แสดงทันที</TooltipContent>
                </Tooltip>
              </div>
            </TooltipProvider>
            <p className="text-xs text-muted">ต้องมี <Code>{"<TooltipProvider>"}</Code> ห่อไว้ · ใช้ <Code>{"side='top|bottom|left|right'"}</Code> · delay default 400ms</p>
          </div>
        </Section>
        ...
        <Section title="Achievement HoverCard">
          <div className="bg-surface rounded-2xl border border-border p-5 space-y-4">
            <p className="text-xs text-muted">
              Hover เพื่อดูรายละเอียด · <span className="font-medium text-foreground">กดเพื่อ pin</span> (กดนอกหรือกดซ้ำเพื่อปิด)
            </p>
            <div className="flex flex-wrap gap-3">
              {DEMO_BADGES.map(b => (
                <ShowcaseBadge
                  key={b.id}
                  badge={b}
                  pinnedId={achievePinned}
                  onPin={setAchievePinned}
                  onUnpin={() => setAchievePinned(null)}
                />
              ))}
            </div>
            <p className="text-xs text-muted">
              <Code>{"open={pinned || hovered}"}</Code> — HoverCard + click-to-pin · กด badge อื่นจะ unpin อัตโนมัติ
            </p>
          </div>
        </Section>
              ...
              <DropdownContent open={dropdownOpen} origin="top left" className="absolute left-0 top-full mt-2 w-44 z-50">
                {["ตัวเลือก 1", "ตัวเลือก 2", "ตัวเลือก 3"].map(l => (
                  <button key={l} onClick={() => { toast.success(l); setDropdownOpen(false); }}
                    className="w-full text-left px-4 py-2.5 text-sm text-secondary hover:bg-elevated transition-colors">{l}</button>
                ))}
                <div className="border-t border-border mt-1 pt-1">
                  <button onClick={() => setDropdownOpen(false)}
                    className="w-full text-left px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors">ยกเลิก</button>
                </div>
              </DropdownContent>
            </div>
      ...
      {/* ════════════════════════════════════════════════════════ */}
      <SectionGroup label="Feedback & Dialogs">
        ...
        <Section title="Toast Notifications">
          <div className="bg-surface rounded-2xl border border-border p-5 flex flex-wrap gap-2">
            <button onClick={() => toast.success("บันทึกสำเร็จ")}
              className="px-4 py-2 rounded-xl bg-emerald-500 text-white text-sm font-medium hover:bg-emerald-600 transition-colors">Success</button>
            <button onClick={() => toast.error("เกิดข้อผิดพลาด")}
              className="px-4 py-2 rounded-xl bg-rose-500 text-white text-sm font-medium hover:bg-rose-600 transition-colors">Error</button>
            <button onClick={() => toast("ข้อความทั่วไป")}
              className="px-4 py-2 rounded-xl bg-elevated border border-border text-sm text-secondary hover:bg-border transition-colors">Default</button>
            <button onClick={() => toast.loading("กำลังโหลด…")}
              className="px-4 py-2 rounded-xl bg-sky-500 text-white text-sm font-medium hover:bg-sky-600 transition-colors">Loading</button>
        ...
        <Section title="Alert Dialog (Confirm)">
          <div className="bg-surface rounded-2xl border border-border p-5 flex items-center gap-4 flex-wrap">
            ...
            <AlertDialog
              ...
              title="ยืนยันการลบ?" description="การกระทำนี้ไม่สามารถย้อนกลับได้"
              danger confirmLabel="ลบ" cancelLabel="ยกเลิก"
              ...
              <div className="bg-surface rounded-b-2xl border border-border border-t-0 p-6 space-y-3">
                ...
                <div className="flex gap-2 justify-end pt-2">
                  <button onClick={() => setModalOpen(false)}
                    className="px-4 py-2 rounded-xl text-sm text-muted bg-elevated hover:bg-border transition-colors">ปิด</button>
                  <button onClick={() => { setModalOpen(false); toast.success("กดยืนยัน"); }}
                    className="px-4 py-2 rounded-xl bg-orange-500 text-white text-sm font-medium hover:bg-orange-600 transition-colors">ยืนยัน</button>
                </div>
              </div>
            </Modal>
          </div>
      ...
      {/* ════════════════════════════════════════════════════════ */}
      <SectionGroup label="Animation & Loading">
