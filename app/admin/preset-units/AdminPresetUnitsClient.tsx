...
export default function AdminPresetUnitsClient({ units: initial }: { units: PresetUnit[] }) {
  ...
  const startEdit = (u: PresetUnit) =>
    setEditing({ id: u.id, unit_name_th: u.unit_name_th, unit_name_en: u.unit_name_en, sort_order: u.sort_order });
  ...
  const saveEdit = () => {
    ...
    startTransition(async () => {
      ...
      setUnits(prev => prev.map(u => u.id === editing.id ? { ...u, ...editing } : u));
      setEditing(null);
  ...
  const handleDelete = (id: string) => {
    if (!confirm("ลบหน่วยนี้? สูตรที่ใช้อยู่จะเก็บ unitFlex ไว้แทน")) return;
    startTransition(async () => {
  ...
  const saveNew = () => {
    ...
    const nextSort = units.length > 0 ? Math.max(...units.map(u => u.sort_order)) + 1 : 1;
    startTransition(async () => {
      ...
      if ("error" in res) { toast.error(res.error); return; }
      setUnits(prev => [...prev, res as PresetUnit]);
  ...
  return (
    <div className="space-y-4">
      <div className="bg-surface rounded-2xl border border-border overflow-hidden">
        {/* Header row */}
        <div className="grid grid-cols-[2rem_1fr_1fr_4rem_5rem] gap-3 px-4 py-2.5 border-b border-border bg-elevated">
          <span />
        ...
        {units.map(u => (
          <div key={u.id}
               className="grid grid-cols-[2rem_1fr_1fr_4rem_5rem] gap-3 items-center px-4 py-2.5 border-b border-border last:border-0 hover:bg-elevated/50 transition-colors">
            <GripVertical className="w-4 h-4 text-muted/40 mx-auto" />

            {editing?.id === u.id ? (
              <>
                <input
                  value={editing.unit_name_th}
              ...
              <>
                ...
                <span className="text-sm text-muted text-center">{u.sort_order}</span>
                <div className="flex gap-1 justify-end">
        ...
        {/* Add new row inline */}
        {adding && (
          <div className="grid grid-cols-[2rem_1fr_1fr_4rem_5rem] gap-3 items-center px-4 py-2.5 border-b border-border bg-orange-50/50 dark:bg-orange-900/10">
            <span />
            <input
              value={newRow.unit_name_th}
      ...
      <p className="text-xs text-muted">
        มี {units.length} หน่วย · หน่วยที่ถูกใช้ในสูตรอยู่แล้วจะไม่ถูกลบออกจาก ingredient_rows
      </p>
    </div>
  );
}
