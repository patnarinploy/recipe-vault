...
import { Clock, Users, Tag, Globe } from "lucide-react";
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        <div className="flex gap-3 text-xs text-stone-400 pt-1">
          {recipe.cook_time_minutes && (
            <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{recipe.cook_time_minutes} นาที</span>
          )}
          {recipe.servings && (
            <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" />{recipe.servings} ที่</span>
          )}
        </div>
      </div>
    </Link>
