...
// OPTIONAL ENRICHMENT ONLY — not the primary source of Version History.
//
// lib/build-history.json is auto-generated by .git/hooks/pre-push on every push
// and is the authoritative record of builds (build number, timestamp, commit
// subject, hash, category). That file updates itself — no human edits needed.
//
// Entries here add a human-readable Thai title and bullet-point details on top
// of the auto-generated card. If a build has no entry here, the page still shows
// its commit subject and category badge from build-history.json automatically.
//
// To enrich a build: add an entry with matching `build` number. Newest first.
